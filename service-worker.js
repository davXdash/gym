...
const CORE=[
 ...
 './js/training-mode-v30.js','./js/coach-v31.js','./js/studio-page-v35.js','./js/device-photo-v36.js','./js/feature-v46.js','./js/supabase-config.js'
];
...
async function prepareHtml(response){
 ...
 html=html.replace('</body>','<script type="module" src="js/bootstrap-v46.js?v=47"></script></body>');
...
self.addEventListener('fetch',event=>{
  ...
  event.respondWith((async()=>{
   ...
   const refresh=(async()=>{
    try{
     ...
     if(fresh.ok)await cache.put('./index.html',(await prepareHtml(fresh)).clone());
    }catch{}
   })();
 ...
 event.respondWith((async()=>{
  ...
  if(cached){
   event.waitUntil(fetch(event.request,{cache:'no-store'}).then(r=>{if(r.ok)return cache.put(event.request,r.clone())}).catch(()=>{}));
   return cached;
  }
  const fresh=await fetch(event.request,{cache:'no-store'});
