...
function ensureDialog(){
  ...
  q('#m69-no',d).onclick=()=>{q('#m69-question',d).hidden=true;q('#m69-form',d).hidden=false;q('#m69-date',d).value=(()=>{const x=pd(today());x.setDate(x.getDate()+1);return new Intl.DateTimeFormat('en-CA',{timeZone:'Europe/Berlin'}).format(x)})()};
...
function maybeAsk(){
  ...
  const code=expectedCode(s);q('#m69-title',d).textContent=`Training ${code} vom ${fmt(item.date)}`;q('#m69-copy',d).textContent=`Für ${fmt(item.date)} war Training ${code} geplant, aber es ist kein abgeschlossenes Training gespeichert. Hast du trainiert?`;q('#m69-question',d).hidden=false;q('#m69-form',d).hidden=true;q('#m69-status',d).textContent='';d.showModal();
}
...
async function saveMissed(d){
  ...
  const remove=active.filter(x=>x.date<date);
  let future=active.filter(x=>x.date>=date);
  const byCode=Object.fromEntries(Object.values(s.workouts||{}).map(w=>[w.code,w.id]));
  const hasDate=future.some(x=>x.date===date);
  const assignments=[];
  if(!hasDate){assignments.push({id:null,date,code,plan_workout_id:byCode[code]});}
  future.forEach((row,i)=>{const offset=i+(hasDate?0:1),newCode=offset%2===0?code:other(code);assignments.push({id:row.id,date:row.date,code:newCode,plan_workout_id:byCode[newCode]})});
  if(hasDate){assignments.sort((a,b)=>a.date.localeCompare(b.date));assignments.forEach((a,i)=>{a.code=i%2===0?code:other(code);a.plan_workout_id=byCode[a.code]})}
  const removeIds=new Set(remove.map(x=>String(x.id)));
  s.schedule=(s.schedule||[]).filter(x=>!removeIds.has(String(x.id)));
  for(const a of assignments){if(a.id){const row=s.schedule.find(x=>String(x.id)===String(a.id));if(row){row.code=a.code;row.plan_workout_id=a.plan_workout_id;row.date=a.date;row.scheduled_date=a.date;row.status='planned'}}else{s.schedule.push({id:`local-v69-${Date.now()}`,date:a.date,scheduled_date:a.date,code:a.code,plan_workout_id:a.plan_workout_id,status:'planned'})}}
  write(SNAP,s);renderHero();
  if(!online()){status.textContent='Offline kann die Kalenderkorrektur nicht sicher gespeichert werden. Bitte später erneut öffnen.';button.disabled=false;return}
  try{
    for(const row of remove){if(!row.id||String(row.id).startsWith('local-'))continue;const r=await supabase.from('scheduled_workouts').delete().eq('id',row.id);if(r.error)throw r.error}
    for(const a of assignments.filter(x=>x.id)){const r=await supabase.from('scheduled_workouts').update({plan_workout_id:a.plan_workout_id,scheduled_date:a.date,status:'planned'}).eq('id',a.id);if(r.error)throw r.error}
    for(const a of assignments.filter(x=>!x.id)){const u=await user();const r=await supabase.from('scheduled_workouts').insert({user_id:u.id,plan_workout_id:a.plan_workout_id,scheduled_date:a.date,status:'planned'});if(r.error)throw r.error}
...
function interceptStart(e){const b=e.target.closest('#start-workout');if(!b)return;const s=snap(),item=actionable(s),code=expectedCode(s);e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();if(item?.date<today()){maybeAsk();return}q(`#workout-list [data-workout="${code}"]`)?.dispatchEvent(new MouseEvent('click',{bubbles:true,cancelable:true,view:window}))}
function install(){renderHero();[250,700,1500].forEach(ms=>setTimeout(()=>{renderHero();maybeAsk()},ms));const hero=q('.hero');if(hero)new MutationObserver(()=>queueMicrotask(renderHero)).observe(hero,{subtree:true,childList:true,characterData:true})}
