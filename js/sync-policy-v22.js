...
function updateSyncLabel(){
  ...
  button.textContent=count?`Sync (${count})`:'Sync';
...
async function requestSync(reason='manual'){
  ...
  try{
    // Die vorhandenen App-Handler führen Queue- und Tracking-Synchronisierung aus.
    button.dispatchEvent(new MouseEvent('click',{bubbles:true,cancelable:true,view:window}));
    await new Promise(resolve=>setTimeout(resolve,1200));
  }finally{
    syncInFlight=false;updateSyncLabel();scheduleRetry();
  }
}
...
document.addEventListener('visibilitychange',()=>{
  if(document.visibilityState==='visible')requestSync('visible');else clearRetry();
});
window.addEventListener('storage',e=>{
  if([QUEUE_KEY,TRACKING_PENDING_KEY,OFFLINE_KEY].includes(e.key)){updateSyncLabel();scheduleRetry()}
});
...
window.addEventListener('load',()=>{
  updateSyncLabel();
  // Ein Versuch beim Start; danach nur Ereignisse oder 60-Sekunden-Retry bei offenen Daten.
  setTimeout(()=>requestSync('startup'),800);
});
